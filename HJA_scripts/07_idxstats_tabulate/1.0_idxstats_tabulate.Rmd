...
(idxstatsgenomecovfolder <- paste0("outputs_F2308_f0x2_", samtoolsqual, "_minimap2_outputs_20200917_kelpie_20200916_BF3BR2_derep_filter3_vsearch97_rmdup_spikes"))

# this is where i store the outputs of this script
(outputidxstatstabulatefolder <- "outputs_minimap2_20200917_F2308_q48_kelpie20200916_BF3BR2_vsearch97")
...
# 1283-1211 = 72 additional species
species_list <- idx %>% 
...
# sanity check:  all count values should be 1285, except for HOBO-357 and HOBO-036, which have ambiguous metadata
species_list <- idx %>% 
  count(site, trap, period) %>% 
  arrange(desc(n))
```
...
```{r sanity checks}
# sanity check:  all count values should be 1285, except for HOBO-357-M1-S2 because there were two samples with this name, and for HOBO-036 because we can't differentiate the periods
...
plan(multisession) # uses all available cores
# plan(multisession, workers = 4) # uses 4 cores
tictoc::tic()
...
# sanity check:  all count values should be 1285, except for HOBO-357_M1_S2 and HOBO-036_M1_S2, which have two copies each (the two S2s should be S1 and S2, but we don't know which one), and thus counts of 2570
...
# F2308 samtools filter
  # saveRDS(genomecoverages_summ, file.path("..", "..", "Kelpie_maps", outputidxstatstabulatefolder, "genomecoverages_summ_F2308_minimap2_20200917_kelpie20200916.RDS"), compress = TRUE)
  # to read in genomecov files
  # genomecoverages_summ <- readRDS(file.path("..", "..", "Kelpie_maps", outputidxstatstabulatefolder, "genomecoverages_summ_F2308_minimap2_20200917_kelpie20200916.RDS"))
...
# sanity check:  all count values should be 1283 OTUs (1285 - the 2 spikes), except for HOBO-357_M1_S2 and HOBO-036_M1_S2, which have 2566 OTUs (because two samples have been pooled in each site)
...
# I also found one error, a species represented by two OTUs in 'kelpie_20200916_BF3BR2_derep_filter3_vsearch97_rmdup_spikes.fas', because i failed to remove the small OTU in the taxonomy stage:
# R2125-7__Insecta_Diptera_Asilidae_NA_NA_BOLD_ABW2452_size=3745
# R2332-6__Insecta_Diptera_Asilidae_NA_NA_BOLD_ABW2452_size=1
# R2125-7 does get mapped to, but i wonder if this OTU would have attracted more reads without the small OTU. I need to experiment. Recall that these are 97% vsearch OTUs, so they are different at the barcode level. Also, they receive mappings, which means that minimap2 sees the sequences as sufficiently different to map reads to them with mapqual >= 48.
species_list <- idx_meta_genomecov %>% 
  filter(COI_Species != "COI_Spike") %>% 
...
# ideally, there should be 1285 in all rows of column "n" 
# 1285 == 1283 species + 2 spike-in species
...
# F2308 samtools filter
# write_tsv(idx_meta_genomecov_corr, gzfile(file.path("..", "..", "Kelpie_maps", outputidxstatstabulatefolder, "idx_meta_genomecov_corr_F2308_minimap_20200916_kelpie20200917.txt.gz"))) # output file
# F2308_f0x2 samtools filter
...
# F2308 samtools filter
# idx_meta_genomecov_corr <- read_delim(file.path("..", "..", "Kelpie_maps", outputidxstatstabulatefolder, "idx_meta_genomecov_corr_F2308_minimap_20200916_kelpie20200917.txt.gz"), "\t", escape_double = FALSE,
#    col_types = cols(
...
envdataMS <- read_csv(file.path("..", "10_eo_data", "sjsdm_biodiversity2_site_info_multispectral_2020-04-13.csv")) %>% 
...
# write output
# F2308 samtools filter
# write_csv(idx_meta_genomecov_filtered_wide, file.path("..", "..", "Kelpie_maps", outputidxstatstabulatefolder, "sample_by_species_table_F2308_minimap2_20200916_kelpie20200917.csv"))
# F2308_f0x2 samtools filter
  ...
  # read_csv() commands
      # mapped_reads
      # F2308 samtools filter
      # idx_meta_genomecov_filtered_wide <- read_csv(file.path("..", "..", "Kelpie_maps", outputidxstatstabulatefolder, "sample_by_species_table_F2308_minimap2_20200916_kelpie20200917.csv"))
...
# library(janitor)
# tabyl(idx_meta_genomecov_corr_filtered_wide, SiteName, trap, period, show_na=FALSE, show_missing_levels = FALSE)
# idx_meta_genomecov_corr_filtered_wide %>% distinct(SiteName) %>% count()
# species_list <- idx_meta_genomecov_corr_filtered_wide %>% 
...
cd ~/Dropbox/Working_docs/Luo_Mingjie_Oregon/HJA_analyses_Kelpie/HJA_scripts/05_taxonomy/kelpie_20200916_${primer}/


vsearch --usearch_global kelpie_20200916_${primer}_derep_filter3_vsearch97_rmdup_spikes.fas --db kelpie_20200916_${primer}_derep_filter3_vsearch97_rmdup_spikes.fas --self --id .84 --iddef 1 --userout match_list.tsv -userfields query+target+id --maxaccepts 0 --query_cov .9 --maxhits 10
```
...
match_list <- read_delim(file.path("..", "05_taxonomy", glue("kelpie_20200916_{primer}"), "match_list.tsv"), "\t", escape_double = FALSE, col_names = FALSE, trim_ws = TRUE)
...
otuhist <- hist(species$occurrences, breaks = 1162, xlim = c(0, 100))

...
This README.text file includes the basic information about the dataset (FOLDER) coded as
...
incidence_lidar_mulspec_5_sample_by_species_table_F2308_minimap2_20200929_kelpie20200927.csv
quasiP_lidar_mulspec_5_sample_by_species_table_F2308_minimap2_20200929_kelpie20200927.csv
...
mean.NDVI.scale, mean.EVI.scale, mean.green.scale, mean.bright.scale, mean.wet.scale (continuous):  multispectral satellite data (mean normalized difference vegetation index, enhanced vegetation index, greenness, brightness, wetness), scaled. The 'mean' refers to the mean value over several measurements of multiple days.
...
Pilot model 2 (multispectral):  Route_x, Route_y, mean.EVI, mean.wet
Pilot model 3 (LiDAR):  Route_x, Route_y, l_Cover_2m_4m, l_Cover_2m_max (variables of xxx and its corresponding xxx_all have almost exactly same effect) 

Please decide if variables or their scaled versiosn should be used according to the distributions that HMSC are using. 
...
# TRAITS (Tr)
The data is in:
'TP_lidar_mulspec_5_sample_by_species_table_F2308_minimap2_20200929_kelpie20200927.csv'
We extracted taxonomic data of class, order, family, genus, species of all OTUs. Order of OTUs are consistent with that in 'SXY' datasets. Information of the OTUs that are missing is indicated as 'NA' in the dataset.
...
# DO YOU POTENTIALLY WISH TO COLLABORATE WITH YOUR DATA ANALYSES AT THE LEVEL OF CO-AUTHORSHIP: MAYBE # change to "NO" or "MAYBE" as needed 

...
resFolder <-"code_sjSDM/r20210716d/results"
...
otu.pa.csv <- otuenv %>% 
  ...
  tidyr::pivot_wider(names_from = trap, values_from = value) %>% ## convert to 1, 0 
  mutate(M1M2 = M1|M2, .keep = "unused") %>% # create single presence/absence for joint M1 M2 samples
  tidyr::pivot_wider(names_from = OTU, values_from = M1M2, values_fn = function(x) ifelse(is.na(x) |!x, 0, 1)) %>%
  select(-SiteName) %>%
  select(which(colSums(.) >= minocc)) %>%
  as.matrix()
...
env.vars <- allVars %>%
  filter(SiteName %in% all_of(unique(otuenv$SiteName[otuenv$period == "S1"]))) %>%
  left_join(y = unique(otuenv[, c("UTM_E", "UTM_N", "SiteName")]), by = "SiteName") %>%
...
## Save model data
save(otu.pa.csv, otuenv, env.vars, fold.id,

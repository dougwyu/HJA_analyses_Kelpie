...
However, there are two corrections of the OTU read numbers still to be done: correcting for the proportion of the total lysis buffer (lysis_ratio) that was used from each sample and correcting for the number of COI DNA spike in reads (COISpike_sum). These two corrections are needed to remove row noise and thus to recover within-species abundance information. 
...
```{r}
OTUs <- idx_meta_genomecov_filtered_lysis %>% 
  select(contains("__")) 

OTU_mins <- as_tibble(apply(OTUs, 2, min))
colSums(OTU_mins) == 0

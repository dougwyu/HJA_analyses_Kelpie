...
spChoose <- "M1" # only M1
# spChoose <- "M1M2" # only shared species
...
# Filter M1S1 for environmental variables
trap <- "M1"
period <- "S1"
otuenv <- otuenv %>% 
...
# for only shared species between M1 and M2
if(spChoose == "M1"){
  # just M1 species
  spM <- sp.chk %>%
    filter(M1) %>%
    select(OTU)
} else if(spChoose == "M1M2") {
...
## get species incidences for plotting later
spIncid <- colSums(otu.pa.csv)
min(spIncid)

spIncid <- data.frame(OTU = names(spIncid), incidence = spIncid)
head(spIncid)

# rm(minocc)

# clean up
rm(datFile, gitHub, kelpierundate, minimaprundate, outputidxstatstabulatefolder, primer, samtoolsfilter, samtoolsqual, fn, sp.chk)
...
## Save model data
save(otu.pa.csv, otu.qp.csv, otuenv, env.vars, spChoose, spIncid,
...
#     ## SAve each model
#     saveRDS(model.train,
#             file.path(resFolder,paste0("s-jSDM_tuning_model_", varsName, "_", spChoose, "_", k, "CV_", 
...
#             file = file.path(resFolder,paste0("manual_tuning_sjsdm_", varsName, "_", k, 
#                                               "CV_", spChoose, 
#                                               "_meanEVAL_", 
#                                               abund, 
#                                               "_min",

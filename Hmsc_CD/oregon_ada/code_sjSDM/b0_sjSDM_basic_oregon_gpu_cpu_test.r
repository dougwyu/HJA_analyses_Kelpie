...
model_cpu_lm <- sjSDM(Y = otu.pa.minocc,
...
model_gpu_lm <- sjSDM(Y = otu.pa.minocc,
                   env = linear(data = env.vars.scale,
                                formula = ~be10+B11_median+mean.EVI+insideHJA + Ess + ht + ht.r500 +
                                  cov4_16 + cov4_16.r500 + mTopo), # linear model on env covariates
                   spatial = linear(data = Sp.data.scale, formula = ~0+UTM_E:UTM_N), # interactions of coordinates
...
model_cpu_DNN <- sjSDM(Y = otu.pa.minocc,
...
model_gpu_DNN <- sjSDM(Y = otu.pa.minocc,
                   env = DNN(data = env.vars.scale,
                                formula = ~be10+B11_median+mean.EVI+insideHJA + Ess + ht + ht.r500 +
                                  cov4_16 + cov4_16.r500 + mTopo), # linear model on env covariates
                   spatial = linear(data = Sp.data.scale, formula = ~0+UTM_E:UTM_N), # interactions of coordinates
...
# 
# # coefficients
coef_cpu_lm <- coef(model_cpu_lm)
coef_gpu_lm <- coef(model_gpu_lm)

coef_cpu_DNN <- coef(model_cpu_DNN)
coef_gpu_DNN <- coef(model_gpu_DNN)

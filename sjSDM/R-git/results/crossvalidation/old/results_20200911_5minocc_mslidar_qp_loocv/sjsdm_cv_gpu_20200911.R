...
# set variables
rundate <- 20200911 # run date
minocc <- 5 # minimum occupancy (incidence) per OTU
envvar <- "mslidar" # gismslidar, gis, ms, lidar, mslidar
...
# read in data
# env data:  scale.env1, should be scaled
scale.env1 <- read_csv(here(datafolder, "scale.env1.csv"))

# species data: otu.data.pa.csv, otu.data.qp.csv, qp == quasiprob, pa == 0/1

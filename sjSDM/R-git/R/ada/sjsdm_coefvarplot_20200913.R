...
minsize <- 1001
maxsize <- 200000 # default 200000 to include all large OTUs

p1 <- coef.figure(summary.p, result, minsize, maxsize)

p1 

pdf(file = here("results", "crossvalidation", resultsfolder, 
                glue("coef_figure_min{minsize}_max{maxsize}_{rundate}.pdf")),
    width = 20, height = 11)
print(p1)
dev.off()

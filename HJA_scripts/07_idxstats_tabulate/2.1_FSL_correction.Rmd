...
idx_meta_genomecov_filtered_wide <- read_csv(here("..", "..", "Kelpie_maps",
...
spike-reads and lysis buffer correction factors. Elaterid and MOrdellid spikes were added in a 9:1 ratio
```{r}
...
idx_meta_genomecov_filtered_lysis_FSL <- idx_meta_genomecov_filtered_lysis %>% 
  mutate(across(contains("__"), ~ .x /(spike_wt_mean*lysis_ratio))) %>% 
...
# idx_meta_genomecov_filtered_lysis_FS_test <- idx_meta_genomecov_filtered_lysis %>%
#   mutate(across(contains("__"), ~ .x /spike_wt_mean)) %>% 
#   mutate(across(contains("__"), ~ .x /lysis_ratio)) 
# comparedfout <- summary(comparedf(idx_meta_genomecov_filtered_lysis_FSL, idx_meta_genomecov_filtered_lysis_FSL_test)) # no values different
...
hist(idx_meta_genomecov_filtered_lysis_FSL$`R1240-70__Insecta_Diptera_Syrphidae_Blera_scitula_BOLD_ABY7981_size=998`)
...
remove some columns, and log and scale environmental variables
```{r}
colselect <- function(inputdf) {
  outputdf <- inputdf %>%
    select(-lysisBufferml, -aliquot, -lysis_ratio, 
         -COISpikeElaterid, -COISpikeMordellid, 
         -spike_wt_mean, -lysis_batch, -site_trap_period
         ) %>%
    relocate(insideHJA, oldGrowthIndex, .after = clearcut)
  return(outputdf)
  }

logandscale <- function(inputdf) {
  outputdf <- inputdf %>%
    mutate(across(c(distToRoad_m, distToStream_m, YrsSinceDist), ~log(.x + 0.001))) %>% 
...
#test code onlylog <- function(inputdf) {
#   outputdf <- inputdf %>%
#     mutate(across(c(distToRoad_m, distToStream_m, YrsSinceDist), ~log(.x + 0.001))) 
#   return(outputdf)
# }

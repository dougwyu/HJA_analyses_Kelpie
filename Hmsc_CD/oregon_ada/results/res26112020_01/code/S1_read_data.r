...
sum(colSums(Y.train)>5)
sum(colSums(Y.train)>10)
...
preds <- c("clearcut","insideHJA","oldGrowthIndex","elevation_m","canopyHeight_m", "precipitation_mm","distToStream_m","mean.NDVI","mean.EVI","mean.green","mean.wet","l_p25","l_rumple","B1_mean","B4_mean","lg_DistRoad","lg_YrsDisturb","lg_cover2m_max","lg_cover2m_4m","lg_cover4m_16m","YrsDist_gt00")
...
X.train$YrsDist_gt00 <- factor(X.train$YrsDist_gt00, levels = c(FALSE, TRUE), labels = c("no", "yes"))
...
# clean up 
rm(dist, spp, X.all, c, cols, dups, files, i, ind, preds, sppCols, tax.cols)


## Save now in pipeline

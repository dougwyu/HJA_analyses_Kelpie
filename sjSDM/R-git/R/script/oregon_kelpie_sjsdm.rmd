...
# format OTU data, combine with multispectral data 
# (remote sensing data)
...
hyper.s1_m1_spike.relAbun = sjSDM_cv(
	Y = as.matrix(as.data.frame(
	  dplyr::select(dataI.1.spike.rel.abun2, contains('__')))
	  ),
	...
	CV=3L, 
	tune_steps = 25L, 
	iter = 60L # iter -> optimization step; lambda -> regularization strength (multiplied constant)
	)
...
#save result
result = list(beta = coef(sjsdm.s1_m1_spike.relAbun), 
              sigma = getCov(sjsdm.s1_m1_spike.relAbun), 
              history = sjsdm.s1_m1_spike.relAbun$history, 
              p = p, 
              logLik=logLik(sjsdm.s1_m1_spike.relAbun)
              )
...
Old code, don't run. just here for backup
```{r sjsdm-model old version, eval=FALSE, include=FALSE}
# old sjsdm version, even syntax of model changed!
# just for backup! don't bother to run!
# code moved down below

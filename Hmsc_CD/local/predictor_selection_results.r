...
samples <- 50
# All But one
load(paste0("results/predSelection/prRes_allB_cv_thin", thin, "_samp", samples, ".rdata"))
# user    system   elapsed 
# 2.974     0.444 26780.803  - same as below..
# id=1096121 Name=Hmsc_pred_uni Ended, Run time 07:26:25
# Job_id=1144812 Name=Hmsc_pred_allB Ended, Run time 22:27:01, COMPLETED, ExitCode 0
...
pdf("results/predSelection/beta_plots.pdf")
...
## could add some error bars to the plots.. or just do boxplots... instead of bars... 

sapply(MF.uni, function(x) sapply(x, sd))
...
pred.allB.CV <- all.res %>%
  filter(metric == "AUC" & type == "all_but" & eval == "MFCV") %>%
  arrange(value)

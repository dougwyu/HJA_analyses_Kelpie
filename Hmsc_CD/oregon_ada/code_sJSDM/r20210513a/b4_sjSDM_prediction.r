...
## Load new data for prdiction and new sacled data
...
pred.mn = apply(abind::abind(lapply(1:5, function(i) {
  predict(modFull_sp, newdata = newData.sc, SP = newXY.sc)}
),along = -1L), 2:3, mean)


pred.sd = apply(abind::abind(lapply(1:5, function(i) {
  predict(modFull_sp, newdata = newData.sc, SP = newXY.sc)}
),along = -1L), 2:3, sd)


save(pred.mn, pred.sd, file = file.path(resFolder, "predictions.rdata"))

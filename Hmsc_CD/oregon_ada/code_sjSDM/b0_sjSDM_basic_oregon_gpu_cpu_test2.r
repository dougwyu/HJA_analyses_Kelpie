...
model_cpu_lm1 <- sjSDM(Y = otu.pa.minocc,
...
model_cpu_lm2 <- sjSDM(Y = otu.pa.minocc,
                      env = linear(data = env.vars.scale,
                                   formula = ~be10+B11_median+mean.EVI+insideHJA + Ess + ht + ht.r500 +
                                     cov4_16 + cov4_16.r500 + mTopo), # linear model on env covariates
                      spatial = linear(data = Sp.data.scale, formula = ~0+UTM_E:UTM_N), # interactions of coordinates
...
model_gpu_lm1 <- sjSDM(Y = otu.pa.minocc,
                   env = linear(data = env.vars.scale,
                                formula = ~be10+B11_median+mean.EVI+insideHJA + Ess + ht + ht.r500 +
                                  cov4_16 + cov4_16.r500 + mTopo), # linear model on env covariates
                   spatial = linear(data = Sp.data.scale, formula = ~0+UTM_E:UTM_N), # interactions of coordinates
...
model_gpu_lm2 <- sjSDM(Y = otu.pa.minocc,
                       env = linear(data = env.vars.scale,
                                    formula = ~be10+B11_median+mean.EVI+insideHJA + Ess + ht + ht.r500 +
                                      cov4_16 + cov4_16.r500 + mTopo), # linear model on env covariates
                       spatial = linear(data = Sp.data.scale, formula = ~0+UTM_E:UTM_N), # interactions of coordinates
...
model_cpu_DNN1 <- sjSDM(Y = otu.pa.minocc,
                   env = DNN(data = env.vars.scale,
                                formula = ~be10+B11_median+mean.EVI+insideHJA + Ess + ht + ht.r500 +
                                  cov4_16 + cov4_16.r500 + mTopo), # linear model on env covariates
                   spatial = linear(data = Sp.data.scale, formula = ~0+UTM_E:UTM_N), # interactions of coordinates
                   se = TRUE, family=binomial("probit"), sampling = 1000L, iter = 100L,
...
model_cpu_DNN2 <- sjSDM(Y = otu.pa.minocc,
                       env = DNN(data = env.vars.scale,
                                 formula = ~be10+B11_median+mean.EVI+insideHJA + Ess + ht + ht.r500 +
                                   cov4_16 + cov4_16.r500 + mTopo), # linear model on env covariates
                       spatial = linear(data = Sp.data.scale, formula = ~0+UTM_E:UTM_N), # interactions of coordinates
...
model_gpu_DNN1 <- sjSDM(Y = otu.pa.minocc,
                   env = DNN(data = env.vars.scale,
                                formula = ~be10+B11_median+mean.EVI+insideHJA + Ess + ht + ht.r500 +
                                  cov4_16 + cov4_16.r500 + mTopo), # linear model on env covariates
                   spatial = linear(data = Sp.data.scale, formula = ~0+UTM_E:UTM_N), # interactions of coordinates
...
model_gpu_DNN2 <- sjSDM(Y = otu.pa.minocc,
                       env = DNN(data = env.vars.scale,
                                 formula = ~be10+B11_median+mean.EVI+insideHJA + Ess + ht + ht.r500 +
                                   cov4_16 + cov4_16.r500 + mTopo), # linear model on env covariates
                       spatial = linear(data = Sp.data.scale, formula = ~0+UTM_E:UTM_N), # interactions of coordinates
...
# 
# # coefficients
coef_cpu_lm1 <- coef(model_cpu_lm1)
coef_gpu_lm1 <- coef(model_gpu_lm1)
coef_cpu_lm2 <- coef(model_cpu_lm2)
coef_gpu_lm2 <- coef(model_gpu_lm2)

coef_cpu_DNN1 <- coef(model_cpu_DNN1)
coef_gpu_DNN1 <- coef(model_gpu_DNN1)
coef_cpu_DNN2 <- coef(model_cpu_DNN2)
coef_gpu_DNN2 <- coef(model_gpu_DNN2)


save(model_cpu_lm1, model_gpu_lm1, coef_cpu_lm1, coef_gpu_lm1,
     model_cpu_DNN1, model_gpu_DNN1, coef_cpu_DNN1, coef_gpu_DNN1,
     model_cpu_lm2, model_gpu_lm2, coef_cpu_lm2, coef_gpu_lm2,
     model_cpu_DNN2, model_gpu_DNN2, coef_cpu_DNN2, coef_gpu_DNN2,
     file ="results_sjSDM/cpu_gpu_cf2.rdata")

...
prRes_allB <- foreach(i = seq_along(preds),
                 .combine=list,
                 .multicombine=TRUE,
                 .packages = c("Hmsc", "coda")) %dopar% {
                   ...
                   partition <- createPartition(m1, nfolds = 2)
                   predsCV = try(computePredictedValues(m1, partition=partition, nParallel = 1))
                   if(inherits(predsCV, "try-error")) MFCV1 <- NA else {
                     MFCV1 <- evaluateModelFit(hM=m1, predY=predsCV)
                   }
                   
                   
                   res <- list(psrf.beta1, MF1, MFCV1)
                   names(res) <- c(paste0("beta_", preds[i], "_all_but"),
                                   paste0("MF_",preds[i], "_all_but"),
                                   paste0("MFCV_",preds[i], "_all_but"))
...
save(prRes_allB, file = paste0("results/predSelection/prRes_allB_cv_thin", thin, "_samp", samples, ".rdata"))

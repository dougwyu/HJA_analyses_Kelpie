...
for(i in 1:k){
  ...
  ## SAve each model
  saveRDS(model.train,
          file.path(resFolder,paste0("s-jSDM_final_model_nsp_", varsName, "_", k, "CV_", 
                                     i, "_", abund, ".rds")))
  ...
  for (pred in 1:2) {
    ...
    pred.dd = apply(abind::abind(lapply(1:3, function(i) {
      predict(model.train, newdata=newdd, SP=NULL)}
    ),along = -1L), 2:3, mean)
...
save(eval.results, sp.mn.train, sp.mn.test, sp.res.test, sp.res.train, file = file.path(resFolder, "nsp_results.rdata"))

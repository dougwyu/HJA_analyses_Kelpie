...
The output of 1.1_idxstats_tabulate.Rmd is a sampleXOTU table from a kelpie run and the associated environmental covariates. However, there are two corrections that are needed: correcting for the proportion of the total lysis buffer (lysis_ratio) that was used from each sample and the number of COI DNA spike in reads (COISpike_sum). These two corrections are needed to remove row noise and thus to recover within-species abundance information. 
...
2. for sjSDM. FSL means log(FSL) transformed using the SPIKEPIPE instructions (see below for details). qp means quasiprobability transformed using scales::rescale()

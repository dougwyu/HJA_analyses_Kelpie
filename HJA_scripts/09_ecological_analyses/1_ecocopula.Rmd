...
minocc <- 2 # set to high number (e.g. 20) for testing
...
env.scale.csv <- env.scale.csv %>%
  select(oldGrowthIndex, elevation_m, canopyHeight_m, minT_annual, distToRoad_m, distToStream_m, YrsSinceDist, mean.NDVI, mean.green, l_Cover_2m_4m, l_Cover_2m_max, l_Cover_4m_16m, l_p25, l_p95, l_rumple)
...
p0 <- ggplot() + 
  ...
  geom_point(aes(x = Factor1, y = Factor2, color = elevation_m, size = YrsSinceDist), data = site_res) + # shape=as.factor(insideHJA), 
...
```{r}
p1 <- ggplot() + 
  geom_point(aes(x = UTM_E, y = UTM_N, color = Factor1), 
             data = site_res, size = 5) +
  geom_text_repel(aes(x = UTM_E, y = UTM_N, label = otuenv$SiteName), data = site_res, size = 2) +
  # geom_label_repel(aes(x = Factor1 * alpha, y = Factor2 * alpha, label = species), data = sp_res, size = 2.5) +
  scale_color_gradientn(colours = brewer.pal(n = 10, name = "PuOr")) +
  theme_classic() + # or "PuOr" or "RdYlBu"
  labs(title = "HJA, Manyany, XY, Factor1")

ggsave(filename = here("..", "..", "Kelpie_maps", 
                outputidxstatstabulatefolder,
                glue("ggsave_ecoCopula_{trap}{period}_minimap2_{minimaprundate}_kelpie{kelpierundate}_{abund}.pdf")), p1)

p2 <- ggplot() + 
  geom_point(aes(x = UTM_E, y = UTM_N, color = Factor2), 
             data = site_res, size = 5) +
  geom_text_repel(aes(x = UTM_E, y = UTM_N, label = otuenv$SiteName), data = site_res, size = 2) +
  # geom_label_repel(aes(x = Factor1 * alpha, y = Factor2 * alpha, label = species), data = sp_res, size = 2.5) +
  scale_color_gradientn(colours = brewer.pal(n = 10, name = "PuOr")) +
  theme_classic() + # or "PuOr" or "RdYlBu"
  labs(title = "HJA, Manyany, XY, Factor2")

...
resFolder <-"code_sjSDM/r20210311c/results"
...
spM <- otuenv %>% 
  ...
  filter(M1) %>% # CHOOOSE HERE FOR SHARED OR SINGEL TRAP SPECIES GFROUP
  select(OTU)

# filter species here to those in sp.M1m2$OTU - already filtered for minocc
otu.qp.csv <- otuenv %>% 
  dplyr::filter(period == "S1" & trap == "M1") %>%
  dplyr::select(spM$OTU) ## file above is already qp
...
# clean up
rm(datFile, gitHub, kelpierundate, minimaprundate, outputidxstatstabulatefolder, primer, samtoolsfilter, samtoolsqual, fn, sp.chk, spM)
...
varsName <- "vars4"
vars <- c("be10",  "be500", "tri", "slope", "Nss", "Ess","ndmi_stdDev", "ndvi_p5", "ndvi_p50", "ndvi_p95", "ndmi_p5", "ndmi_p50", "ndmi_p95", "tpi", "ht", "ht.r250", "ht.r500", "ht.r1k", "cov2_4.r250", "cov2_4.r500","cov2_4.r1k", "cov4_16", "cov4_16.r250", "cov4_16.r500", "cov4_16.r1k", "mTopo", "cut.r1k.pt", "insideHJA", "lg_DistStream", "lg_DistRoad", "lg_YrsDisturb", "l_p25", "l_rumple", "minT_annual", "maxT_annual", "precipitation_mm")
...
lambda.env = seq(0,1, length.out=4)	# .1
alpha.env = seq(0,1, length.out=4)		# .9
lambda.sp = seq(0,1, length.out=4)	# .1 ## Changed to 5
alpha.sp =  seq(0,1, length.out=4)	# .5 ## Changed to 5

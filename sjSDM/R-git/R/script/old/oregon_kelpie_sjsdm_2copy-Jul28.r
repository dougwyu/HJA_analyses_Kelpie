...
lapply(c("ggplot2",'vegan', 'tidyverse','gridBase', 'grid', 'ggcorrplot','here', 'conflicted','reticulate','sjSDM'), library, character.only=T)
...
lrs = seq(0, 1, length.out = 5)
result = vector("list", 5*5)
...
for(i in 1:5) { 
	for(j in 1:5) {
		model = sjSDM( Y = as.matrix(otu.data),
					   ...
					   lambda=lrs[i], alpha=lrs[j] ),
					   biotic = bioticStruct(lambda=lrs[i], alpha=lrs[j], on_diag=F),
					   spatial = linear(data=as.matrix(XY), ~0+UTM_E:UTM_N, 
					   lambda = lrs[i], alpha = lrs[j])
					   )
		  loss=unlist(model$logLik)
		  history=model$history
		  weights = list(regulation=vector(lambda=lrs[i], alpha=lrs[j]), beta = coef(model), sigma = getCov(model),loss=loss,history=history)
		  rm(model)
		  result[[j+(i-1)*5]] = weights
	}
}
  #iter = 1000L, step_size = 6L, sampling = 100L, learning_rate = 0.0003
#saveRDS(result, file = here('R','result','s-jSDM_result_s1_m1_present_no2.RDS') )
	
result = readRDS(here('R','result','s-jSDM_result_s1_m1_present_no2.RDS') )
str(result)
	
# best model with lowest log-likelihood
r.ind=10 # see section below 'r.ind=which(a[1,]==min(a[1,]))'
result[[r.ind]]$regulation
lrs=c(.25,1); i=1; j=2
	
model = sjSDM( Y = as.matrix(otu.data),
			   learning_rate = 0.01, iter = 100L, step_size = 27L, link='probit', sampling = 100L, 
			   env = linear(data=scale.env1,
...
#saveRDS(model, file = here('R','result','s-jSDM_model_s1_m1_present_no2.RDS') )
model = readRDS(here('R','result','s-jSDM_model_s1_m1_present_no2.RDS'))
p = getSe(model)
	
loss=unlist(model$logLik)
weights = list(regulation=c(lambda=lrs[i], alpha=lrs[j]), beta = coef(model), sigma = getCov(model),loss=loss,history=model$history, p=p, logLik=logLik(model))
	
#saveRDS(weights, file = here('R','result','s-jSDM_best_result_s1_m1_present_no2.RDS') )
weights = readRDS(here('R','result','s-jSDM_best_result_s1_m1_present_no2.RDS') )
	
summary.p=summary(weights$p)
str(summary.p)
	
# ... session 1, Malaise I, quasiprob (>2), no mulspec-data, spatial

all.data = dataI.1.rel.abun.no2
otu.data = as.data.frame(select(all.data, contains('__')))
...
formula = ~ elevation.scale + canopy.ht.scale+ min.T.scale + max.T.scale + precipitation.scale + metre.road.scale + metre.stream.scale + yrs.disturb.min.scale, 
		  ...
		  loss=unlist(model$logLik)
		  history=model$history
		  weights = list(beta = coef(model), sigma = getCov(model),loss=loss,history=history, p=p, logLik=logLik(model))
		  rm(model)
		  result[[j+(i-1)*5]] = weights
	}
}
  #iter = 1000L, step_size = 6L, sampling = 100L, learning_rate = 0.0003
saveRDS(result, file = here('R','result','s-jSDM_result_s1_m1_quasiP_no2.RDS') )
...
a = sapply(result, function(x) x$loss)
r.ind=which(a[1,]==min(a[1,]))
	
summary(result[[r.ind]])
plot(result[[r.ind]]$history)
	
result[[r.ind]]$regulation
#  lambda alpha
#1   0.25     1
	
# ... variation partition & analysis of variation
model = readRDS(here('R','result','s-jSDM_model_s1_m1_present_no2.RDS'))
# . plot variation partition
#pdf(here('R','graph','sjsdm_s1_m1_present_no2', 'vari.par_sjSDM_s1_m1_present_no2.pdf'), width=5, height=5)
	
imp =importance(model)
plot(imp, cex=.8) #from s-jSDM package
title( paste('variation partition, ',ncol(result[[r.ind]]$sigma), ' OTUs', sep=''))
	
#dev.off()
	

# . anova.sjSDM
# there's not significant difference between 'anova' of 3_cv & 2_cv
an = anova(model,cv = 3L)
saveRDS(an, file = here('R','result','sjSDM_anova_s1_m1_present_no2.RDS'))
an = readRDS(here('R','result','sjSDM_anova_s1_m1_present_no2.RDS'))
	
#pdf(here('R','graph','sjsdm_s1_m1_present_no2','anova_sjSDM_s1_m1_present_no2.pdf'), width=5, height=5)
	
plot(an)
title(paste('analysis of variance, ',ncol(result[[r.ind]]$sigma), ' OTUs', sep=''))
	
dev.off()
	

# ... correlation plot ...
dim(result[[r.ind]]$sigma)# cov <- result$sigma 
# [1] 437 437
otu.table = as.data.frame(dplyr::select(all.data, contains('__')))
str(otu.table[,1:5])
	
co.env.spp <- cov2cor(result[[r.ind]]$sigma)
rownames(co.env.spp) <- 1:dim(result[[r.ind]]$sigma)[1]   #spp.names
colnames(co.env.spp) <- 1:dim(result[[r.ind]]$sigma)[1]   #spp.names
...
for(i in 1:5) {
	a = result[[1+(i-1)*5]]$regulation[1,1]
	pdf(here('R','graph','sjsdm_s1_m1_present_no2','correlation_plot', paste('species_correlation_', 'lambda', a,'.pdf', sep='')), height=12, width=12)
	#result[[r.ind]]$regulation[1,1]
#	par(mfrow=c(2,3))
	for(j in 1:5) {
		r.ind = j+(i-1)*5
		co.env.spp <- cov2cor(result[[r.ind]]$sigma)
		
		print(ggcorrplot(co.env.spp, hc.order = T, outline.color = "white", insig = "blank",sig.level = 0.05, lab_size = 1,show.legend=T, title=paste('sjSDM version: ',packageVersion('sjSDM'),', kelpie20200214, alpha: ', result[[r.ind]]$regulation[1,2],', lambda: ', result[[r.ind]]$regulation[1,1],sep='')))
...
pdf(here('R','graph','sjsdm_s1_m1_present_no2', paste('species_correlation_', 'alpha', result[[r.ind]]$regulation[1,2], '_lambda', result[[r.ind]]$regulation[1,1],'.pdf', sep='')), height=12, width=12)
	
ggcorrplot(co.env.spp, hc.order = T, outline.color = "white", insig = "blank",sig.level = 0.05, lab_size = 1,show.legend=T, title=paste('sjSDM version: ',packageVersion('sjSDM'),', kelpie20200214, alpha: ', result[[r.ind]]$regulation[1,2],', lambda: ', result[[r.ind]]$regulation[1,1],sep=''))
...
```{r sjsdm-analysis-polygon&envir&spp-space-no-sjsdmcv (0/1)}
# ... session 1, Malaise I, 0/1, tradition-data, space ...
all.data = dataI.1.present.no2
otu.data = as.data.frame(select(all.data, contains('__')))
scale.env1 = all.data[ ,c('elevation.scale','canopy.ht.scale','min.T.scale','max.T.scale','precipitation.scale','metre.road.scale','metre.stream.scale','yrs.disturb.min.scale')]
str(scale.env1)
...
sigma = re_scale(result[[r.ind]]$sigma)[order(apply(otu.tbl, 2, sum)), order(apply(otu.tbl, 2, sum))]
	
#pdf(here('R','graph','sjsdm_s1_m1_present_no2', paste('species_covariance_circular_', 'alpha', result[[r.ind]]$regulation[1,2],'_lambda', result[[r.ind]]$regulation[1,1],'.pdf', sep='')), height=8, width=8)
	
version.text = paste('sjSDM version: ',packageVersion('sjSDM'),', kelpie20200214, alpha: ', result[[r.ind]]$regulation[1,2],', lambda: ',result[[r.ind]]$regulation[1,1] ,sep='')
otu.text = "sum of presence"
	
source(here("R", "source", "sjsdm-analyse-functions.r"))
	
cov.circle(version.text=version.text, otu.text=otu.text, sigma=sigma, otu.tbl=otu.tbl, result=result[[r.ind]])
...
beta = as.matrix(data.frame(result[[r.ind]]$beta$env, result[[r.ind]]$beta$spatial)[,2:10])
effects = apply(beta, 1, function(o) sum(abs(o)))
str(effects)
	
n = ncol(result[[r.ind]]$sigma)# number of otus
...
version.text = paste('sjSDM: ',packageVersion('sjSDM'),', kelpie20200214, alpha: ', result[[r.ind]]$regulation[1,2],', lambda: ', result[[r.ind]]$regulation[1,1], sep='')
...
cov.circle.env(version.text=version.text, evnames=evnames, otu.text=otu.text,result=result[[r.ind]], effect_comb=effect_comb, otu.tbl=otu.tbl)
	
#dev.off()
	

# . bar plot with max.min spp & envir factor
#str(summary.p)
#sum_coef<-data.frame(summary.p$coefmat)
str(effect_comb)
...
effect_ind = inner_join(effect_comb, a1, by=c('otu', 'otu'))
	
beta1 = data.frame(result[[r.ind]]$beta$env, result[[r.ind]]$beta$spatial)[,2:10]
names(beta1) = evnames
...
col = viridis::viridis(length(unique(effect_ind$max_effects)))
	
#pdf(here('R','graph','sjsdm_s1_m1_present_no2','max_envir_20spp_beta.pdf'), height=8, width=15)
	
t= -(abs(effect_ind$beta[which.max(abs(effect_ind1$beta))]))*.93
# as.factor(rank)
ggplot(effect_ind,aes(x = otu, y = beta, fill=max_effects)) +
...
```{r sjsdm-model-noSpace-sjsdmcv (0/1)}
# . sjSDM-version = ‘0.0.4.9000’
# 0/1, >2, no space
all.data = dataI.1.present.no2
otu.data = as.data.frame(select(all.data, contains('__')))
...
formula = ~ elevation.scale + canopy.ht.scale+ min.T.scale + max.T.scale + precipitation.scale + metre.road.scale + metre.stream.scale + yrs.disturb.min.scale, 
  ...
  #iter = 1000L, step_size = 6L, sampling = 100L, learning_rate = 0.0003
saveRDS(result, file = here('R','result','s-jSDM_result_s1_m1_present_no2_noSpace.RDS') )
	
```


```{r sjsdm-model-only-mulspec (0/1)}
...
scale.env1 = data.frame(select(all.data, starts_with('mean')))
str(scale.env1)
...
saveRDS(result, file = here('R','result','s-jSDM_result_s1_m1_present_no2_mulspec.RDS') )
	

sjsdm.s1_m1_present.spec = readRDS(here('R','result','model_sjSDM_s1_m1_spike.present.spec_V0.0.2.9000.RDS'))
	
an = anova(sjsdm.s1_m1_present.spec,cv = 3L)
saveRDS(an, file = here('R','result','anova_sjSDM_s1_m1_spike.present.spec_V0.0.2.9000.RDS'))
	
plot(an)
# there's not significant difference between 'anova' of 3_cv & 2_cv
	
#pdf('anova_sjSDM_s1_m1_spike.present.spec_2cv.pdf', width=5, height=5)
...
# ... session 1, Malaise I, quasiprob ...
# . sjSDM-version = ‘0.0.2.9000’
all.data = dataI.1.rel.abun.no2
otu.data = as.data.frame(dplyr::select(all.data, contains('__')))
	
scale.env1 = select(all.data, ends_with('.scale'), -starts_with('mean.'))
str(scale.env1)
	
hyper.s1_m1.quprob = sjSDM_cv(
	Y = as.matrix(otu.data),
	env = linear(as.matrix(scale.env1)),
	biotic = bioticStruct(on_diag=FALSE), 
	tune='random', n_cores = NULL,
	CV=5L, tune_steps = 20L, iter = 60L 
	# iter -> optimization step; lambda -> regularization strength (multiplied constant)
	)
 saveRDS(hyper.s1_m1.quprob, file = here('R','result','s-jSDM_tune_s1_m1_spike.relAbun_V0.0.3.9000.RDS') )
...
```{r sjsdm-model-only-mulspec (abun)}
# ... session 1, Malaise I, spike, rel.abun ...
# ... use multispectral environmental data
# . sjSDM-version = ‘0.0.3.9000’
names(dataI.1.rel.abun.no2[,c(1:3,24:39)])
...
#[16] "mean.EVI.scale"
scale.env = dataI.1.rel.abun.no2[,c(1:3,24:36)]
str(scale.env)
...
# . sjSDM-version = ‘0.0.2.9000’
scale.env1 = dataI.1.present2[,c('elevation.scale','canopy.ht.scale','min.T.scale','max.T.scale','precipitation.scale','metre.road.scale','metre.stream.scale','yrs.disturb.min.scale','mean.NDVI.scale','mean.EVI.scale', 'mean.green.scale', 'mean.bright.scale', 'mean.wet.scale')]
str(scale.env1)
...
hyper.s1_m1_spike.present.W.spec2 = readRDS(here('R','result','s-jSDM_tune_s1_m1_spike.present.W.spec2_V0.0.2.9000.RDS'))
...
#saveRDS(sjsdm.s1_m1_spike.present.W.spec2, file = here('R','result','model_sjSDM_s1_m1_spike.present.W.spec2_V0.0.2.9000.RDS'))
...
```{r sjsdm-analysis-tenary-with-mulspec2 (0/1)}
# ... session 1, Malaise I, present, with all spec-indices ...
# .. load 'sjSDM_cv' & 'sjSDM' result
model = readRDS(here('R','result','model_sjSDM_s1_m1_spike.present.W.spec2_V0.0.2.9000.RDS'))
tune = readRDS(here('R','result','s-jSDM_tune_s1_m1_spike.present.W.spec2_V0.0.2.9000.RDS') )
result = readRDS(result, file = here('R','result','sjSDM_s1_m1_spike.present.W.spec2_V0.0.2.9000.RDS')) 
	

#plot variation partition
imp =importance(model)
#plot(imp) #from s-jSDM package
vp.plot(imp)# from functions.R
an = anova(model,cv = 2L)
#print(an)
plot(an)
...
# . sjSDM-version = ‘0.0.2.9000’
scale.env1 = dataI.1.present2[,c('elevation.scale','canopy.ht.scale','min.T.scale','max.T.scale','precipitation.scale','metre.road.scale','metre.stream.scale','yrs.disturb.min.scale','mean.NDVI.scale','mean.EVI.scale')]
str(scale.env1)
...
hyper.s1_m1_spike.present.W.spec = readRDS(here('R','result','s-jSDM_tune_s1_m1_spike.present.W.spec_V0.0.2.9000.RDS'))
...
# saveRDS(sjsdm.s1_m1_spike.present.W.spec, file = here('R','result','model_sjSDM_s1_m1_spike.present.W.spec_V0.0.2.9000.RDS'))
	
an = anova(sjsdm.s1_m1_spike.present.W.spec,cv = 3L)
#saveRDS(an, file = here('R','result','anova_sjSDM_s1_m1_spike.present.W.spec_V0.0.2.9000.RDS')) 

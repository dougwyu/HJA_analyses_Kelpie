...
X.train <- env.vars %>%
  ...
  mutate(clearcut = factor(clearcut),
...
spp <- data.frame(species = colnames(Y.train)) %>%
...
rm(list = ls()[!ls() %in% c("Y.train", "X.train", "S.train","P", "resFolder", "modFolder")])
save(Y.train, X.train, S.train, P, file = "data/vif_data.rdata")

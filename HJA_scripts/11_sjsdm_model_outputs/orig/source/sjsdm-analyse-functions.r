...
cov.circle <- function(version.text, otu.text, sigma, otu.tbl, result) {
  ...
  segments(
    x0 = cos(deg2rad(356)) * (lineSeq - 0.2), x1 = cos(deg2rad(356)) * (lineSeq + 0.9),
  ...
  polygon(
    x = c(cos(deg2rad(arrow_angle)) * (lineSeq + 0.5), cos(deg2rad(arrow_angle)) * (lineSeq + 0.7), cos(deg2rad(max(angles))) * (lineSeq + 0.6), cos(deg2rad(arrow_angle)) * (lineSeq + 0.5)),
...
cov.circle.env <- function(version.text, evnames, otu.text, result, effect_comb, otu.tbl) {
  ...
  sppname3 <- seq(1:20)
  sppname3 <- as.character(sppname3)
  ...
  for (i in sort(unique(max_effects))) {
    ...
    x <- c(
      (3.6 + 1.5 * (sub_eff[, 2])) * cos(deg2rad(sub[, 3])),
      rev((3.6 + 1.5 / 2) * cos(deg2rad(sub[, 3])))
    )

    y <- c(
      (3.6 + 1.5 * (sub_eff[, 2])) * sin(deg2rad(sub[, 3])),

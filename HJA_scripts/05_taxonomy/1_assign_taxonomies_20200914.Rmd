...
here()
gbifotufolder <- "kelpie_20200927_BF3BR2"
...
# 1,538 OTUs, min 400, avg 418, max 486
```
...
If the alignment looks like it could do with a second re-alignment, first use seqkit seq -g to remove gaps from the fasta file

```{bash}
seqkit seq -g dna.aligned.revtrans.fasta > dna.aligned.revtrans2.fasta

seqkit seq -g dna.aligned.revtrans3.fasta > dna.aligned.revtrans4.fasta
```

After curation in Geneious or TranslatorX/JALview, rename the fasta file

```{bash}
mv dna.aligned.revtrans4.fasta kelpie_20200927_BF3BR2_derep_filter3_vsearch97.fas
```
...
# TO ADD LATER:  CHECK THAT ALL CONSENSUSCLASSIFICATIONS ARE UNIQUE. OCCASIONALLY, MULTIPLE OTUS HAVE THE SAME SIZE VALUE AND SLICE_MAX() DOES NOT REDUCE THEM TO ONE OTU
...
```{bash }
cat ../../08_reference_sequences_datasets/assembled_plasmids.fasta kelpie_20200927_BF3BR2_derep_filter3_vsearch97_rmdup.fas  > kelpie_20200927_BF3BR2_derep_filter3_vsearch97_rmdup_spikes.fas

cat kelpie_20200927_BF3BR2_derep_filter3_vsearch97_rmdup_spikes.fas | less
# check that the first two sequences are separated correctly from the OTU seqs

seqkit seq -w 80 -u  kelpie_20200927_BF3BR2_derep_filter3_vsearch97_rmdup_spikes.fas > kelpie_20200927_BF3BR2_derep_filter3_vsearch97_rmdup_spikes_format.fas

mv kelpie_20200927_BF3BR2_derep_filter3_vsearch97_rmdup_spikes_format.fas kelpie_20200927_BF3BR2_derep_filter3_vsearch97_rmdup_spikes.fas

seqkit stats kelpie_20200927_BF3BR2_derep_filter3_vsearch97_rmdup_spikes.fas
# format  type  num_seqs  sum_len  min_len  avg_len  max_len
# FASTA   DNA      1,227  512,490      402    417.7      897
```

NEXT STEP
Use kelpie_20200927_BF3BR2_derep_filter3_vsearch97_rmdup_spikes.fas as the mapping target to generate the OTU table. Upload to ~/_Oregon/2019Sep_shotgun/reference_seqs and change the $REFSEQS value

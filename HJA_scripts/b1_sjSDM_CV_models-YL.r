...
#### YL code --- 
###  filter species for minocc based on training
require('vegan')
alldata2 = otuenv %>% filter(period == 'S1' & SiteName %in% train.Names)
spM = data.frame(min = specnumber(select(alldata2, contains('__')), MARGIN=2)>=minocc) %>% rownames_to_column(var='OTU') %>% filter(min==T) %>% select(OTU)
rm(alldata2)
...
ran.tt = data.frame(table(env.vars$SiteName) )
colnames(ran.tt) = c('SiteName', 'NumTrap')
num.fold = table(rep_len(1:k, length.out = nrow(otu.pa.csv))) %>% as.vector()
fold.id = data.frame(site=character(), fold=character(), NumTrap=numeric())

for (i in 1:(k-2)) {
	sel = 0; iseq=vector()
	while ( sel != num.fold[i] ) {
		x1 = num.fold[i]
		x2 = ceiling(num.fold[i]/2)
		if (sum(ran.tt$NumTrap==2)<x2) { x2 = x1-sum(ran.tt$NumTrap==2) }
		
		num1 = base::sample(x1:x2, 1)
		iseq = base::sample( ran.tt$SiteName, num1)
		sel = sum(ran.tt$NumTrap[ran.tt$SiteName %in% iseq])
	}
	dd = data.frame(site=iseq, NumTrap=ran.tt$NumTrap[ran.tt$SiteName %in% iseq], fold=as.character(i))
	fold.id = fold.id %>% bind_rows(., dd)
	ran.tt = ran.tt[!(ran.tt$SiteName %in% iseq), ]
	rm(dd, iseq)
	print(i)
}
seq = vector(); sel1 = 0; sel2 = 0 
while (sel1 != num.fold[k] & sel2 != num.fold[k-1]) {
	x1 = num.fold[k]
	x2 = ceiling(num.fold[k]/2)
	if (sum(ran.tt$NumTrap==2)<x2) { x2 = x1-sum(ran.tt$NumTrap==2) }
	
	num1 = base::sample(x1:x2, 1)
	seq = base::sample( ran.tt$SiteName, num1)
	sel1 = sum(ran.tt$NumTrap[ran.tt$SiteName %in% seq])
	sel2 = sum(ran.tt$NumTrap[!(ran.tt$SiteName %in% seq)])
}
fold.id = fold.id %>% bind_rows(., data.frame(site=ran.tt$SiteName[!(ran.tt$SiteName %in% seq)], NumTrap=ran.tt$NumTrap[!ran.tt$SiteName %in% seq], fold=as.character(k-1))) %>% bind_rows(., data.frame(site=seq, NumTrap=ran.tt$NumTrap[ran.tt$SiteName %in% seq], fold=as.character(k))) 
	
sapply(unique(fold.id$fold), function(a) sum(fold.id$NumTrap[fold.id$fold==a])); num.fold
sort(unique(fold.id$site)) == sort(unique(train.Names))
	
rm(num.fold, sel, sel1, sel2, x1, x2, num1, seq, ran.tt)
	
fold.id = fold.id$fold[ sapply(env.vars$SiteName, function(a) which(fold.id$site==a)) ]

...
(outputidxstatstabulatefolder_test <- glue("outputs_minimap2_{minimaprundate}_F2308_q48_kelpie{kelpierundate}_{primer}_vsearch97"))
...
plan(multisession) # uses all available cores, 188 secs
...
tictoc::toc() 
# rbind the list into a dataframe when using future_map()
# genomecoverages_summ <- do.call(rbind, genomecoverages_summ) 


# THIS STEP REQUIRES several mins for barcodes if using purrr:map()
...
species <- select(idx_meta_genomecov_filtered_wide, contains(c("Vespidae"))) %>% colSums() %>% as.data.frame() %>% rownames_to_column(var = "taxon"); tibble::view(species)
...
```{r prepare lulu dataframe}
names(idx_meta_genomecov_filtered_wide)

# a data.frame that has sites/samples as columns and OTUs (unique OTU id's) as rows, and observations as read counts

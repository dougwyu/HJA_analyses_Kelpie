...
# pre transform (???) -- thinking about presence absence data rather than the probs.. 
Xtb <- vegan::decostand(Xmat, method = "hellinger")
Xtb[1:10, 1:10]

system.time(
  pca <- vegan::rda(X = Xmat, scale = T)
)

system.time(
  tb_pca <- vegan::rda(X = Xtb, scale = T)
)
...
tbPca1 <- makeR(r, vegan::scores(tb_pca, 1, "sites"), NAs)
tbPca2 <- makeR(r, vegan::scores(tb_pca, 2, "sites"), NAs)

# plot(stack(pcaR1, pcaR2, tbPca1, tbPca2))
...
save(pca, r, NAs, file = file.path(resFolder, "ord_res.rdata"))

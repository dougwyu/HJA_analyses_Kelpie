...
cd kelpie_20200916_LERAY
seqkit split -s 900 kelpie_20200916_LERAY_derep.fas 
```
...
After curation in Geneious or TranslatorX/JALview, save the new fasta file (kelpie_20200916_LERAY_derep_filter2.fas)
...
# 1,619 OTUs, min 402, avg 416.8, max 426
```
...
# open kelpie_otus2df.csv in Excel and manually filter OTUs so that only the largest one matching to a BOLDID is retained (more details above). Save as kelpie_otus2df_rmdup.csv
# on 20 Sep 2020, i learned about dplyr::slice_max(), which lets one do this more or less automatically, but it is not incorporated into this pipeline
...
# FASTA   DNA      1,285  536,586      402    417.6      897
```

...
scale.env <- allVars.sc[allVars.sc$period == "S1",vars]
nrow(scale.env)
head(scale.env)
...
scale.XY <- as.matrix(xy.sites.sc[xy.sites.sc$period == "S1", c("UTM_E", "UTM_N")])
nrow(scale.XY)
...
save(pred.mn.cl, pred.sd.cl, file = file.path(resFolder, paste0("sjSDM_predictions_", "M1S1_", "min", minocc, "_", varsName, "_", abund, "_clamp", ".rdata")))
